...
typedef struct _LEAP_CONFIG_CHANGE_EVENT {
  ...
  // The status of the commit operation, true on success, false on failure
  bool status;
...
/**
 * Opens a device reference and retrieves a handle
 *
...
typedef struct _LEAP_BONE {
  ...
  // Rotation in world space from forward.  Convert to a matrix for basis vectors.
...
typedef struct _LEAP_DIGIT {
  ...
  union {
    ...
    struct {
      ...
      // Note that this is anatomically incorrect; it's actually the intermediate phalange
      // that's absent in a real thumb.
      // We set metacarpal to 0 instead for ease of programming.
      LEAP_BONE metacarpal;

      // The bone closest to the knuckle, on the outside of the palm
      LEAP_BONE proximal;

      // The bone between the Proximal bone and the fingertip.
...
enum eLeapEventType {
  ...
  /**
   * An error has occurred for a image request,
...
/**
 * Retrieves the number of bytes required to allocate a frame identified by the specified ID
 *
 * @param hConnection A handle to the connection object
...
/**
 * Constructs an interpolated frame given the specified timestamp
 *
 * @param hConnection A handle to the connection object
 * @param timestamp The timestamp for which an interpolated frame is to be obtained
...
// An opaque rebase state structure:
typedef struct _LEAP_CLOCK_REBASER *LEAP_CLOCK_REBASER;

/**
 * Creates a new Leap clock-rebaser object
 *
 * This method must be called before this member is passed to any other routine.
 *
 * @param phClockRebaser Receives a pointer to a clock-rebaser object
 */
 ...
 *
 * This relationship is neither fixed nor stable.  Simulation restarts can cause user clock values to change
 ...
 *
 * @param userClock A clock value supplied by the user, sampled at about the same time as LeapGetNow was sampled
 * @param leapClock The clock value sampled by a call to LeapGetNow
 */
LEAP_EXPORT eLeapRS LEAP_CALL LeapUpdateRebase(LEAP_CLOCK_REBASER hClockRebaser, int64_t userClock, int64_t leapClock);

/**
 * Computes the leap clock corresponding to a specified user clock value
 *
 * @param userClock The clock referenced in user clock time
 ...
 *
 * @param hConnection A handle to the clock-rebaser object to be destroyed
 */
